function [tc,amp,rss,npx,id,tcpx,amppx,rsspx,nps] = cmprt_ana4(v, ...
                             mask,rsls,nrsls,time,ntime,fun,init,t0,opt)
%CMPRT_ANA4  Given a four-dimensional matrix of T1/T2 intensities from
%          a MRI image volume, a mask, and a vector of spin lock/echo
%          times, calculates the T1rho/T2* map for the volume.
%
%          TC = CMPRT_ANA4(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT) Given
...
%          (lateral and medial) and two bones (femur and tibia) (the
%          cartilage layers are combined) in array TC.
%
%          TC = CMPRT_ANA4(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT,T0) If
...
%
%          TC = CMPRT_ANA4(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT,T0,OPT)
%          a structure, OPT, with options for the curvefit solver.
%          See Matlab command "optimset" for a list of parameters.
%
%          [TC,AMP,RSS,NPX,ID] = CMPRT_ANA4(...) returns the amplitudes
...
%          lateral and 1 - medial), and the second column being bone
%          (0 - femur and 1 - tibia) that identifies the analyzes, ID. 
%
%          [TC,AMP,RSS,NPX,ID,TCPX,AMPPX,RSSPX,NPS] = CMPRT_ANA4(...)
%          returns the T1rho/T2* for each pixel in the two compartments
%          (lateral and medial) and two bones (femur and tibia) in a
%          cell array, TCPX, the exponential amplitudes for each pixel
%          in a cell array, AMPPX, the sum of squared residuals for
%          each pixel, RSSPX, and the number of pixels in each slice
