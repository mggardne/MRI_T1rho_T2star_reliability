...
% ddir = fullfile('..','CACL MRI Master'); % For CACL Subjects
div = uigetdir;
ddir = fullfile(div); 
...
%
%  bfnam = fullfile(ddir,snam,'013_L_tibiaCS.mat');    % Bone MAT file name for FFE
   bfnam = fullfile(ddir,snam,'Tibia','013_L_tibiaCS.mat');  % Bone MAT file name for RHO 
   d = dir(bfnam);
   if isempty(d)
     error([' *** ERROR in tcart08:  Not able to find tibia bone ', ...
            'file:  ',bfnam,'\n']);
   else
 % bfnam = fullfile(ddir,snam,d.name);    % Bone MAT file name For FFE
    bfnam = fullfile(ddir,snam,'Tibia',d.name);    % Bone MAT file name For Rho
   end
...
%   cfnam = fullfile(ddir,snam,[kid '_tibiaCart.mat']);     % Cartilage MAT file name for FFE
   cfnam = fullfile(ddir,snam,'Tibia',[kid '_tibiaCart.mat']);     % Cartilage MAT file name For Rho
   if ~exist(cfnam,'file')
...
%
   zgl(:,k) = gridproj(tril,xyzl,xgl,ygl,1,dist);
   zgm(:,k) = gridproj(trim,xyzm,xgm,ygm,1,dist);
%
   xyzgl = [xgl ygl zgl(:,k)];
   xyzgm = [xgm ygm zgm(:,k)];
%
...
save(rnam,'cthkls','cthkms','ilegs','kids', ...
     'snams','tribl','tribm','trilss','trimss', ...
     'xyzbl','xyzbm','xyzils','xyzims', ...
     'xyzlss','xyzmss','zgl','zgm');
%
