function bones = rd_roiso(rdir,leg,ld,itroch,irho)
%RD_ROISO  Reads a particular loaded or unloaded leg sagittal
...
%
%          BONES = RD_ROISO(RDIR,LEG,LD) given the directory name in the
...
%
%          BONES = RD_ROISO(RDIR,LEG,LD,ITROCH) given the
%          logical ITROCH, trochlear ROIs are returned in the femur
%          structure BONES(1).
%
%          BONES = RD_ROISO(RDIR,LEG,LD,ITROCH,IRHO) given the
...
%                  without "_RO.".
%
%                  7.  Cartilage (SAGAR) and bone (SAG) CSV files with
%                  "_ORG" in the file names are used in place of the
%                  same files without "_ORG".
%
%                  8.  Trochlea option is NOT tested.
%
%          15-Mar-2024 * Mack Gardner-Morse 
%
...
%
if (nargin<3)
  error(' *** ERROR in RD_ROISO:  Three inputs are required!');
end
%
if (nargin<4)
  itroch = false;
...
%    end
   ...
   if nrfiles>2
     fprintf(1,'  %s\n',rnams{:});
     error(' *** ERROR in RD_ROISO:  Too many CSV files found!');
   end
%
