function [tc,amp,rss,npx,id,tcpx,amppx,rsspx,nps] = cmprt_ana4u(v, ...
                             mask,rsls,nrsls,time,ntime,fun,init,t0,opt)
%CMPRT_ANA4U  Given a four-dimensional matrix of T1/T2 intensities from
%          a MRI image volume, a mask, and a vector of spin lock/echo
%          times, calculates the T1rho/T2* map for the volume.
%
%          TC = CMPRT_ANA4U(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT) Given
...
%          index to lateral and medial compartments, MASK, a cell array
%          with the slices within each compartment, RSLS, the number of
...
%          the two compartments (lateral and medial) (the two cartilage
%          layers [superficial and deep] are combined) in array TC.
%
%          TC = CMPRT_ANA4U(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT,T0) If
...
%
%          TC = CMPRT_ANA4U(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT,T0,
%          OPT) a structure, OPT, with options for the curvefit solver.
%          See Matlab command "optimset" for a list of parameters.
%
%          [TC,AMP,RSS,NPX,ID] = CMPRT_ANA4U(...) returns the amplitudes
%          of the exponential fits, AMP, the sum of squared residuals,
%          RSS, the number of pixels in the curvefits, NPX, and a
%          column array for the compartment (0 - lateral and 1 - medial)
%          that identifies the analysis, ID. 
%
%          [TC,AMP,RSS,NPX,ID,TCPX,AMPPX,RSSPX,NPS] = CMPRT_ANA4U(...)
%          returns the T1rho/T2* for each pixel in the two compartments
%          (lateral and medial) in a cell array, TCPX, the exponential
...
%
if nargin<8
  error([' *** ERROR in cmprt_ana4u:  Eight input variables are', ...
         ' required!']);
end
%
if nargin<9
...
%
tc = zeros(2,1);        % Time constant - T1rho/T2*
amp = zeros(2,1);       % Amplitude of exponential
rss = zeros(2,1);       % Sum of squared residuals
npx = zeros(2,1);       % Number of pixels in regional fit
id = zeros(2,1);        % Compartment
%
nps = cell(2,1);        % Number of pixels in each slice
tcpx = cell(2,1);       % Time constant - T1rho/T2* for each pixel
amppx = cell(2,1);      % Amplitude of exponential for each pixel
rsspx = cell(2,1);      % Sum of squared residuals for each pixel
%
...
%
   ...
   msk = mask{kr};      % Mask for this compartment
