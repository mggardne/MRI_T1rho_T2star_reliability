function roi = rd_roi6hp(filenam,ipx)
%RD_ROI6HO  Reads an OSIRIX ROI CSV file.
%
%         ROI = RD_ROI6HP(FILENAM) reads the OSIRIX ROI CSV file,
%         FILENAM, and returns the structure, ROI, with the names of
%         the ROI in field "name" and the X, Y and Z data points for
%         each slice is in the columns of cell arrays in field "data".
%         The X, Y and Z data is in ordered triplets.  Each triplet
%         represent a point from a slice in the ROI.  Each ROI is
%         returned in a row in the structure.  
%
%         ROI = RD_ROI6HP(FILENAM,IPX) if IPX is true (nonzero) the X
%         and Y pixel data points for each slice is in the columns of
%         cell arrays in field "data" in the structure ROI.  Also
%         returns the field "imageno" with the image (or slice) number.
%
%         NOTES:  1.  The data was collected from OSIRIX using the
%                 polygon tool.
%
%                 2.  A 0.5 pixel is added to all of the OsiriX pixel
%                 coordinates to match the indexing into the image
%                 matrix.
%
%                 3.  If the ROI name does not start with an alphabetic
%                 letter, the letter "a" is prepended to the ROI name.
%
%         24-Jun-2024 * Mack Gardner-Morse
%
...
%
if (nargin<1)
  error(' *** ERROR in RD_ROI6HP:  An input file name is required!');
end
%
if (nargin<2)
  ipx = false;
