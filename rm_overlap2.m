%#######################################################################
%
%            * ReMove OVERLAPs in Segmentations 2 Program *
%
%          M-File which reads the MS-Excel spreadsheet file,
%     contact_chk2.xlsx, to find the segmentation CSV files and slices
...
%     files rm_overlap*.ps in the Results\RemoveOverlap2 folder.
%
...
%             rd_rois.m, rd_rois2.m, trnsf2mm.m and trnsf2pixel.m
...
%
resdir = fullfile('Results','RemoveOverlap2');   % Results directory
psnam = fullfile(resdir,'rm_overlap2_');    % Start of PS file name
pstyp = '.ps';          % PS file type
%
% Read Overlap Data from contact_chk2.xlsx
%
% csvdat
%     Column 1:  Subject
%     Column 2:  Visit
...
%     Column 5:  Load
%
datdir = fullfile('Results','ContactChecks2');
xlsnam = 'contact_chk2.xlsx';
% sht = 'Combined Data';
sht = 1;
...
%
   ...
   if ko==8
     slsk = slsk(1:2);  % Remove tibia CSV file slices for Subject 08 on Visit 1
   end
...
%
   if ko==2             % "RO" CSV file
     ...
     csvf = get_csv(cdirf,leg,ld,false);    % Femur CSV file
   end
   csvt = get_csv(cdirt,leg,ld,false);      % Tibia CSV file
%
...
%
% Fix Gap in Femur Coordinates for Subject 5 on T2* Visit 2 Right Leg
% on Slice 19
%
      if ko==6&&ks==6
        fpts3d = fix_gap(fpts3d);
        fpts2d = fix_gap(fpts2d);
      end
%
