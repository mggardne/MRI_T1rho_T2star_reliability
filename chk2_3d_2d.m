%#######################################################################
%
% * CHecK2 of 3D and 2D Coordinates in Segmentation CSV Files Program *
%
...
%     "_RO2" in the filenames (created by rm_overlapMay24.m).  If the
%     3D and 2D coordinates do not match, the CSV filename is printed
%     to the screen.
%
...
%
% Find RO2 Cartilage Segmentations
%
            dro = dir(rdirk);
            dro = {dro(~[dro.isdir]').name}';    % File names
            idr = contains(dro,'_RO2');
            dro = dro(idr);
            nfiles = size(dro,1);
%
