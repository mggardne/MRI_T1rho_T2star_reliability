function [tri,xy,nt] = mk_tri_2df(dat,dist,tol,iplt)
...
%        be bone.
%
%        [TRI,XY] = MK_TRI_2DF(DAT) given a cell array containing two
%        (2) columns matrices with boundary line coordinate point data,
%        DAT, returns a three (3) column triangle connectivity matrix,
%        TRI, and X and Y coordinates in a two columns matrices XY.
%
%        [TRI,XY,NT] = MK_TRI_2DF(DAT) returns the number of triangles,
%        NT.
%
%        [TRI,XY] = MK_TRI_2DF(DAT,DIST) given a distance, DIST, ensures
...
%
%                4.   M-files near2.m, lsect2.m, and lsect2a.m must be
%                in the current directory or path.
%
%                5.  Similar to mk_tri_2d.m except the slopes of the
...
%
if (nargin<1)
  error(' *** ERROR in mk_tri_2df:  No input data!');
end
%
if nargin<2
...
%
if nslice~=2
  error([' *** ERROR in mk_tri_2df:  Input cell array must ', ...
