%#######################################################################
%
%            * MRI FIT Reliability Tread Analysis Program *
%
...
%     mri_fitrt.xlsx, in the "Results\Tread" directory.
%
...
%
%             4.  M-file to read the meniscus segmentation MAT files
...
%
resdir = fullfile('Results','Tread');  % Results directory
%
ifirst = true;          % First write to file
xlsnam = 'mri_fitrt.xlsx';             % Results spreadsheet
...
hdrs2 = {'Pixels' 'T1R/T2S' 'RSS' 'ValidPix' 'Mean' 'Min' 'Max' ...
         'SD' 'COV'};
%
psnam = fullfile(resdir,'mri_fitrt_'); % Start of PS file name
...
%
         ...
         [~,idb] = intersect(rsl,rslb);     % Index to both compartments
%
...
%
         maskfl = maskf(:,:,idl);      % Lateral
         maskfm = maskf(:,:,idm);      % Medial
%
         masktl = maskt(:,:,idl);      % Lateral
...
%
         ...
         maskfb = maskfl|maskfm;       % Combine femur compartment masks
         masktb = masktl|masktm;       % Combine tibia compartment masks
         maskb = {maskfb; masktb; maskfb|masktb};     % Combine femur and tibia masks
%
...
%
      if nstar~=nroi
        error([' *** ERROR in mri_fitra2:  Number of T2* MAT files', ...
...
%
if ~iskip
  save(fullfile(resdir,'mri_fitrt.mat'),'t1r_res','t1r_npx', ...
