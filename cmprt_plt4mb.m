function cmprt_plt4mb(v,mask,rsls,nrsls,idt,tcp,nps,mxtc,cmap,bx, ...
                     txt1,psnam)
%CMPRT_PLT4MB  Plots T1rho/T2* values with the underlying images by
%          slice within regions of interest (ROIs).
%
%          CMPRT_PLT4MB(V,MASK,RSLS,NRSLS,IDT,TCP,NPS) Given a four-
...
%
%          CMPRT_PLT4MB(V,MASK,RSLS,NRSLS,IDT,TCP,NPS,MXTC,CMAP,BX,TXT1)
%          Given the maximum plotting value for the color scale, MXTC,
%          a three color map, CMAP, an array defining a plot bounding
%          box in pixels, BX, and a text string for the first line or a
%          cell array with text for the plot title, TXT1, plots the
%          T1rho/T2* values with a color maximum of MXTC using the
%          color map, CMAP, within the plot window BX and using TXT1
%          for the first line of the plot title.  The default maximum
%          value is 70.  The default color map is gray for the image
%          and jet for the T1rho/T2* values.  The default window is the
%          whole image.  The default first line title text is "Results
%          Plot".
%
%          CMPRT_PLT4MB(V,MASK,RSLS,NRSLS,IDT,TCP,NPS,MXTC,CMAP,BX,TXT1,
...
%
%          NOTES:  1.  Plots the pixel output of cmprt_ana4mb.m.  See
%                  cmprt_ana4mb.m and mri_mer_fit.m.
%
%                  2.  Based on cmprt_plt.m, cmprt_plt4.m, and
%                  cmprt_plt4m.m, but adapted for use with plotting
%                  knee meniscus results, added a bounding box to zoom
%                  in on the meniscus, and using additional masks.
%
...
%
if nargin<7
  error([' *** ERROR in cmprt_plt4mb:  Seven input variables are', ...
         ' required!']);
end
%
...
% Initialize Arrays
%
idxs = [3 3];           % Maximum indices for compartment and position
%
...
%
      for kb = 3        % Combined anterior and posterior positions
%
