function [ips,ts,idc,ierr] = lisect4a(p1,p2,pll,tol,clim)
%LISECT4A Finds the intersections of a 3-D closed line with a 3-D
%         piecewise linear line.
%
%         IPS = LISECT4A(P1,P2,PLL) finds the intersections of a closed
%         line defined by an initial point (P1) and a final point (P2)
%         with a piecewise linear line (PLL).  PLL is defined by a
%         series of 3-D points with the X, Y and Z coordinates of the
%         points in columns.  The X, Y and Z coordinates of the
%         intersections with the piecewise linear line (PLL) are
%         returned in IPS.  IPS is empty if there are no intersections.
%
%         [IPS,TS,IDC] = LISECT4A(P1,P2,PLL) returns the distance along
...
%         (PLL) with the intersection.  The index (IDC) is to the point
%         of the segment in the piecewise linear line (PLL) that
%         intersects the input line.  If there is no intersection, TS
%         and IDC are empty arrays.
%
%         [IPS,TS,IDC,IERR] = LISECT4A(P1,P2,PLL,TOL) sets IERR to true
%         if no intersection is found within tolerance (TOL).  Default
%         tolerance is 1e-8.
%
%         [IPS,TS,IDC,IERR] = LISECT4A(P1,P2,PLL,TOL,CLIM) displays a
...
%
%                 2.  The input points must be of length three (3).  The
%                 piecewise linear line must have three (3) columns.
%
%                 3.  The M-file lisect3.m must be in the current path
%                 or directory.
%
...
%
if (nargin<3)
  error([' *** ERROR in  LISECT4A:  LISECT4A requires three input', ...
         ' arguments.']);
end
%
...
if (n1~=3)||(l1~=1)||(n2~=3)||(l2~=1)||(n3<2)||(l3~=3)
  error(' *** ERROR in LISECT4A:  Error in input arguments.')
end
%
% Initialize Arrays
%
...
%
   [ip,t,ier] = lisect3(p1,v1,p2,v2,tol,clim);
   if ~isempty(ip)&&~ier
     ips(k,:) = ip';
     ts(k,:) = t';
     ierr = false;
