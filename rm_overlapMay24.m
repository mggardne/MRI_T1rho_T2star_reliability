%#######################################################################
%
%        * ReMove OVERLAPs in Segmentations MAY 2024 Program *
%
%          M-File which reads the MS-Excel spreadsheet file,
%     contact_chkMay24.xlsx, to find the segmentation CSV files and
%     slices where the femur and tibia segmentations overlap, reads the
%     femur and tibia segmentation CSV files, finds and adds midpoints
...
%     files have the same name with "_RO2" appended to the name.
%
%          The slices with corrected overlaps are plotted to Postscript
%     files rm_overlapMay24*.ps in the Results\RemoveOverlap folder.
%
...
% Analysis Parameters
%
omin = 0.1;             % Minimum overlap for correction in pixels
%
tol2 = 1e-4;            % Tolerance for 2D intersections
tol3 = 2.5e-2;          % Tolerance for 3D intersections
%
% Print?
%
% iprt = false;
...
psnam = fullfile(resdir,'rm_overlapMay24_');     % Start of PS file name
pstyp = '.ps';          % PS file type
%
% Read Overlap Data from contact_chkMay24.xlsx
%
% csvdat
%     Column 1:  Subject
%     Column 2:  Visit
...
%     Column 5:  Load
%
datdir = fullfile('Results','ContactChecksMay24');
xlsnam = 'contact_chkMay24.xlsx';
% sht = 'Combined Data';
sht = 1;
...
%
% Skip Old Remove Overlap Files (*_RO.csv)
%
lro = false;            % Do NOT read old _RO files
%
...
% Get CSV Files
%
   csvf = get_csv(cdirf,leg,ld,lro);   % Femur CSV file
   csvt = get_csv(cdirt,leg,ld,lro);   % Tibia CSV file
%
...
%
      ...
      if npts~=npts2d
        warning([' *** WARNING in RM_OVERLAPMAY24:  Numbers of 3D', ...
                 ' and 2D points do not match!']);
...
%
   idot = strfind(csvf,'.');
   idot = idot(end);
   csvf = [csvf(1:idot-1) '_RO2' csvf(idot:end)];
   write_csv(cdirf,csvf,flines);
%
   idot = strfind(csvt,'.');
   idot = idot(end);
   csvt = [csvt(1:idot-1) '_RO2' csvt(idot:end)];
