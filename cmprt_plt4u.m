function cmprt_plt4u(v,mask1,rsls1,mask2,rsls2,idt,tcp1,nps1,tcp2, ...
                     nps2,mxtc,cmap,txt1,psnam)
%CMPRT_PLT4U  Plots T1rho/T2* values with the underlying images by slice 
%          within regions of interest (ROIs).
%
%          CMPRT_PLT4U(V,MASK1,RSLS1,MASK2,RSLS2,IDT,TCP1,NPS1,TCP2,
%          NPS2) Given a four-dimensional matrix of T1/T2 intensities
%          from a MRI image volume, V, where the first two dimensions
%          are an image, the third dimension are the slices, and the
%          fourth dimension are the spin lock/echo times, three
...
%
%          CMPRT_PLT4U(V,MASK1,RSLS1,MASK2,RSLS2,IDT,TCP1,NPS1,TCP2,
%          NPS2,MXTC,CMAP,TXT1) Given the maximum plotting value for the
...
%
%          CMPRT_PLT4U(V,MASK1,RSLS1,MASK2,RSLS2,IDT,TCP1,NPS1,TCP2,
%          NPS2,MXTC,CMAP,TXT1,PSNAM)  Given the name for a Postscript
%          (PS) file, PSNAM, prints the plots to the PS file.  By
%          default, the plots are not printed.
%
...
%
if nargin<10
  error([' *** ERROR in cmprt_plt4u:  Ten input variables are', ...
         ' required!']);
end
%
