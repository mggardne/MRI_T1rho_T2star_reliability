function [tc,amp,rss,npx,id,tcpx,amppx,rsspx,nps] = cmprt_ana4mb(v, ...
                             mask,rsls,nrsls,time,ntime,fun,init,t0,opt)
%CMPRT_ANA4MB  Given a four-dimensional matrix of T1/T2 intensities from
%          a MRI image volume, a mask, and a vector of spin lock/echo
%          times, calculates the T1rho/T2* map for the volume.
%
%          TC = CMPRT_ANA4MB(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT)
...
%          dimension are the spin lock/echo times, two dimensional
%          logical masks with the first dimension being the image and
%          the second dimension being the image slices in a cell array
%          of masks with the index to lateral and medial compartments
%          and within the compartments an index to the anterior and
%          posterior positions, MASK, a cell array with the slices
...
%          (lateral, medial, and both) and two positions (anterior,
%          posterior, and both) in array TC.
%
%          TC = CMPRT_ANA4MB(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT,T0)
...
%
%          [TC,AMP,RSS,NPX,ID] = CMPRT_ANA4MB(...) returns the
%          amplitudes of the exponential fits, AMP, the sum of squared
%          residuals, RSS, the number of pixels in the curvefits, NPX,
%          and a two column array with the first column being the
%          compartment (0 - lateral, 1 - medial, and 2 - both), and the
%          second column being position (0 - anterior, 1 - posterior,
%          and 2 - both) that identifies the analyzes, ID. 
%
%          [TC,AMP,RSS,NPX,ID,TCPX,AMPPX,RSSPX,NPS] = CMPRT_ANA4MB(...)
%          returns the T1rho/T2* for each pixel in the two compartments
%          (lateral, medial, and both) and two positions (anterior,
%          posterior, and both) in a cell array, TCPX, the exponential
%          amplitudes for each pixel in a cell array, AMPPX, the sum of
%          squared residuals for each pixel, RSSPX, and the number of
%          pixels in each slice within the regions in a cell array, NPS.
%
%          NOTES:  1.  The T1rho/T2* map is calculated as a mono-
%                  exponential as a function of spin lock/echo times.
%
%                  2.  The image volume data and spin lock/echo times
...
%                  workers.
%
%                  8.  Based on the M-files cmprt_ana.m, cmprt_ana4.m,
%                  and cmprt_ana4m.m, but adapted for use for
%                  calculating T1rho/T2* for the knee meniscus with
%                  combined anterior and posterior masks, and combined
%                  lateral and medial masks.
%
%          27-Jun-2024 * Mack Gardner-Morse
%
...
%
if nargin<8
  error([' *** ERROR in cmprt_ana4mb:  Eight input variables are', ...
         ' required!']);
end
%
if nargin<9
...
%   2 - Positions:  0 - Anterior, 1 - Posterior, and 2 - Both
%
tc = zeros(9,1);        % Time constant - T1rho/T2*
amp = zeros(9,1);       % Amplitude of exponential
rss = zeros(9,1);       % Sum of squared residuals
npx = zeros(9,1);       % Number of pixels in regional fit
id = zeros(9,2);        % Columns: 1 - Compartment and 2 - Position
%
nps = cell(9,1);        % Number of pixels in each slice
tcpx = cell(9,1);       % Time constant - T1rho/T2* for each pixel
amppx = cell(9,1);      % Amplitude of exponential for each pixel
rsspx = cell(9,1);      % Sum of squared residuals for each pixel
%
...
%
   for kb = 1:3
%
      ikb = kb-1;       % Position code identifier (0 - anterior, 1 - posterior, 2 - both)
%
      idx = 3*kr+kb-3;  % Index to variables
      id(idx,:) = [ikr ikb];           % Compartment and position IDs
%
      msk = mskr{kb};   % Mask for this position
...
%
% Calculate T1rho/T2* for Each Pixel
%
      if nargout>5
        tcp = zeros(npx(idx),1);
        ampp = tcp;
