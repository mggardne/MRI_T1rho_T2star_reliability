...
%
%        01-Feb-2021 * Mack Gardner-Morse * Added trap for vertical
%                lines (code lines 57-72).
%
...
for k = 1:ns
   l = k+1;
   if abs(pll(l,1)-pll(k,1))<1e-8
     xcoord(k) = (pll(l,1)+pll(k,1))/2;
     yeq = true;
   else
   ...
   if ((xcoord(k)<=pll(l,1))&&(xcoord(k)>=pll(k,1)))|| ...
     ...
     if yeq
       ycoord(k) = m1*xcoord(k)+b1;
       if ycoord(k)<min(pll([k l],2))||ycoord(k)>max(pll([k l],2))
         xcoord(k) = NaN;
         ycoord(k) = NaN;
       end
     else
