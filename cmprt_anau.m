function [tc,amp,rss,npx,id,tcpx,amppx,rsspx,nps] = cmprt_anau(v, ...
                             mask,rsls,nrsls,time,ntime,fun,init,t0,opt)
%CMPRT_ANAU  Given a four-dimensional matrix of T1/T2 intensities from
%          a MRI image volume, a mask, and a vector of spin lock/echo
%          times, calculates the T1rho/T2* map for the volume.
%
%          TC = CMPRT_ANAU(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT) Given
...
%          the two compartments (lateral and medial), and two layers
%          (superficial and deep) in array TC.
%
%          TC = CMPRT_ANAU(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT,T0) If
...
%
%          TC = CMPRT_ANAU(V,MASK,RSLS,NRSLS,TIME,NTIME,FUN,INIT,T0,OPT)
%          a structure, OPT, with options for the curvefit solver.
%          See Matlab command "optimset" for a list of parameters.
%
%          [TC,AMP,RSS,NPX,ID] = CMPRT_ANAU(...) returns the amplitudes
...
%          lateral and 1 - medial), and the second column being layer
...
%
if nargin<8
  error([' *** ERROR in cmprt_anau:  Eight input variables are', ...
         ' required!']);
end
%
if nargin<9
...
%   2 - Layers:  0 - Deep and 1 - Superficial
%
...
id = zeros(4,2);        % Columns: 1 - Compartment, 2 - Layer
%
...
%
   for kl = 1:2         % 1 - superficial, 2 - deep
%
      idx = 2*kr+kl-2;  % Index to variables
...
%
      ...
      id(idx,:) = [ikr 2-kl];          % Compartment, and layer IDs
%
