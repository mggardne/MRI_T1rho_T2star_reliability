%#######################################################################
%
%        * CONTACT CHecK 2 of Segmentations MAY 2024 Program *
%
...
%     contact_chk*_RO2.ps in the Results\ContactChecks2May24 folder.
%     The maximum overlaps are also saved to the MS-Excel spreadsheet, 
%     contact_chk2May24.xlsx, in the Results\ContactChecks2May24 folder.
%
%     NOTES:  1.  M-files lsect3.m, lsect4.m, lsect5.m, mxd2linssy.m,
%             pts2lin.m, rd_roi6.m and rd_roiso2.m must be in the
...
%
resdir = fullfile('Results','ContactChecks2May24');   % Results directory
%
ifirst = true;          % First write to file
xlsnam = 'contact_chk2May24.xlsx';     % Results spreadsheet
...
% Read Segmentations
%
            brois = rd_roiso2(rdirk,leg,ld,itroch,4);
%
% Get Femur Data
%
...
%
                  if ~isempty(ipts)
%
% Get Unique Set of Intersection Points
%
                    [iptss,idu] = uniquetol(ipts,1e-8,'ByRows',true);
                    idcss = idcs(idu);
%
% Plot Intersections
%
...
%
                    ...
                    plot(tdats(:,1),tdats(:,2),'g.-','LineWidth', ...
                         1.5,'MarkerSize',12);
                    plot(iptss(:,1),iptss(:,2),'ro','LineWidth',1.5, ...
                         'MarkerSize',8);
                    axis equal;
...
%
                    title({['Subject ' subjnam ' ' vnam ' T1\rho']; ...
                          ...
                          'FontWeight','bold');
%
% Check Intersections for Overlaps
%
                    ni = size(ipts,1); % Number of intersections
                    dd = 0;
%
                    for km = 1:ni-1
                       mi = km+1;
...
%
                       if nf>0
                         ...
                         txyz = tdats(idtl,:);
                         fxyz = fdats(idfx,:);
                         [df,idfmx,xyzf] = mxd2linssy(txyz,fxyz,true);
                         if idfmx>0
...
%
                       if nt>0
                         ...
                         txyz = tdats(idtx,:);
                         fxyz = fdats(idfl,:);
                         [dt,idtmx,xyzt] = mxd2linssy(fxyz,txyz,false);
                         if idtmx>0
...
% Read Segmentations
%
            brois = rd_roiso2(rdirk,leg,ld,itroch,5);
%
% Get Femur Data
%
            fc = brois(1).rois(1);     % Femur cartilage ROIs
...
%
psnamh = fullfile(resdir,'overlap_hist_RO2.ps');
